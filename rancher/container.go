...
func (r *RancherContainer) up(start bool) error {
	...
	// TODO: need to the API call to determine if an upgrade is necessary
	// blocked until this is in cattle
	if r.Context().Upgrade {
		_, err := r.Client().Container.ActionUpgrade(existing, &client.ContainerUpgrade{
			Config: *container,
		})
		return err
	} else if r.Context().Rollback {
		_, err = r.Client().Container.ActionUpgrade(existing, &client.ContainerUpgrade{
			RevisionId: existing.PreviousRevisionId,
		})
		return err
	}
